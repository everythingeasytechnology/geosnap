...
const STEPS: {
...
}[] = [
  {
    ...
    description:
      'GeoSnap automatically embeds precise GPS coordinates and your current address into every photo and video you capture.',
  ...
  },
  {
    ...
    description:
      'Access your camera to capture geotagged photos and videos. Microphone access is also requested for video recording with audio.',
...
export default function PermissionsScreen() {
  ...
  const initialStep = Math.min(Math.max(parseInt(params.start ?? '0') || 0, 0), STEPS.length - 1);
  ...
  async function requestCurrentPermission() {
    ...
    if (step.key === 'location') {
      ...
      await Camera.requestMicrophonePermissionsAsync().catch(() => null);
...
const styles = StyleSheet.create({
  ...
  progressRow: { flexDirection: 'row', gap: 8, marginTop: 20, marginBottom: 10 },
  ...
  stepLabel: {
    fontSize: 11, color: '#ADBAF0', letterSpacing: 1.5, fontWeight: '600',
  },

  content: {
    flex: 1, alignItems: 'center', justifyContent: 'center', paddingBottom: 20,
  },
  ...
  description: {
    fontSize: 15, color: '#6E6E6E', textAlign: 'center',
  ...
  allowBtn: {
    ...
    shadowColor: '#3244C2', shadowOffset: { width: 0, height: 4 },
  ...
  allowText: { fontSize: 16, fontWeight: '800', color: '#FFFFFF', letterSpacing: 0.3 },
  ...
  brandText: { fontSize: 12, color: '#ADBAF0', fontWeight: '600', letterSpacing: 0.3 },
});
